...
class PgsqlSchemaConnectionDriverTest extends DatabaseFeatureTestCase
{
    use UsesTenants;
    use UsesConnections;
    ...
    protected function registerDatabaseListener()
    {
        ...
        $this->configureBoth(function ($event) {
            $overrides = $event->defaults($event->tenant);
            $overrides['schema'] = $overrides['database'];
            unset($overrides['database']);
            $event->useConnection('pgsql', $overrides);
        });
    }
}
